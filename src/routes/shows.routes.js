...
// New: current hero
r.get("/hero/current", async (_req, res) => {
  const hero = await Show.findOne({ isHero: true }).lean();
  res.json(hero || null);
});
...
r.post(
  ...
  async (req, res) => {
    ...
    const payload = {
      ...
      // saved URLs that the client can request as /uploads/<file>
      ...
      isHero: body.isHero === "true" || body.isHero === true,
      inHeroReel: body.inHeroReel === "true" || body.inHeroReel === true,
      heroVideo: body.heroTrailerUrl || body.heroVideo || "", // remote mp4 URL allowed
    };

    try {
      // If created as hero, unset any previous
      if (payload.isHero) {
        await Show.updateMany({ isHero: true }, { $set: { isHero: false } });
      }
      const created = await Show.create(payload);
...
/* ---------- admin: set hero + optional trailer url ---------- */
r.post("/:id/hero", requireAdmin, async (req, res) => {
  const { heroVideo, heroVideoUrl } = req.body;
  const newUrl = heroVideoUrl || heroVideo;
...
/* ---------- admin: toggle hero reel ---------- */
r.post("/:id/reel", requireAdmin, async (req, res) => {
  const { inHeroReel, heroVideoUrl } = req.body;
  const set = {};
  if (typeof inHeroReel !== "undefined")
    set.inHeroReel = !!(inHeroReel === true || inHeroReel === "true");
  if (heroVideoUrl) set.heroVideo = heroVideoUrl;
